...
use windows::Win32::UI::Input::KeyboardAndMouse::{VK_ADD, VK_APPS, VK_BACK, VK_BROWSER_BACK, VK_BROWSER_FAVORITES, VK_BROWSER_FORWARD, VK_BROWSER_HOME, VK_BROWSER_REFRESH, VK_BROWSER_SEARCH, VK_BROWSER_STOP, VK_CAPITAL, VK_CLEAR, VK_CONTROL, VK_CONVERT, VK_DECIMAL, VK_DELETE, VK_DIVIDE, VK_DOWN, VK_END, VK_ESCAPE, VK_F1, VK_F10, VK_F11, VK_F12, VK_F13, VK_F14, VK_F15, VK_F16, VK_F17, VK_F18, VK_F19, VK_F2, VK_F20, VK_F21, VK_F22, VK_F23, VK_F24, VK_F3, VK_F4, VK_F5, VK_F6, VK_F7, VK_F8, VK_F9, VK_HANGUL, VK_HELP, VK_HOME, VK_INSERT, VK_KANA, VK_LAUNCH_APP1, VK_LAUNCH_APP2, VK_LAUNCH_MAIL, VK_LCONTROL, VK_LEFT, VK_LMENU, VK_LSHIFT, VK_LWIN, VK_MEDIA_NEXT_TRACK, VK_MEDIA_PLAY_PAUSE, VK_MEDIA_PREV_TRACK, VK_MEDIA_STOP, VK_MENU, VK_MULTIPLY, VK_NEXT, VK_NONCONVERT, VK_NUMLOCK, VK_NUMPAD0, VK_NUMPAD1, VK_NUMPAD2, VK_NUMPAD3, VK_NUMPAD4, VK_NUMPAD5, VK_NUMPAD6, VK_NUMPAD7, VK_NUMPAD8, VK_NUMPAD9, VK_OEM_1, VK_OEM_102, VK_OEM_2, VK_OEM_3, VK_OEM_4, VK_OEM_5, VK_OEM_6, VK_OEM_7, VK_OEM_COMMA, VK_OEM_MINUS, VK_OEM_PERIOD, VK_PAUSE, VK_PLAY, VK_PRINT, VK_PRIOR, VK_RCONTROL, VK_RETURN, VK_RIGHT, VK_RMENU, VK_RSHIFT, VK_RWIN, VK_SCROLL, VK_SELECT, VK_SEPARATOR, VK_SHIFT, VK_SLEEP, VK_SNAPSHOT, VK_SPACE, VK_SUBTRACT, VK_TAB, VK_UP, VK_VOLUME_DOWN, VK_VOLUME_MUTE, VK_VOLUME_UP};
use windows::Win32::UI::WindowsAndMessaging::{CreateWindowExW, DefWindowProcW, DispatchMessageW, GetMessageW, LoadCursorW, RegisterClassExW, ShowWindow, TranslateMessage, CW_USEDEFAULT, HMENU, IDC_ARROW, MSG, SW_SHOWNORMAL, WINDOW_EX_STYLE, WM_KEYDOWN, WNDCLASSEXW, WS_OVERLAPPEDWINDOW};
use crate::keyboard::key::KeyboardKey;
use crate::keyboard::Shared;
use crate::mouse::windows::window_proc as mouse_window_proc;
...
impl KeyboardKey {
    fn from_vk(vk: usize) -> Option<Self> {
        match vk {
            ...
            v if v == 0x30 => Some(KeyboardKey::Num0),
            v if v == 0x31 => Some(KeyboardKey::Num1),
            v if v == 0x32 => Some(KeyboardKey::Num2),
            v if v == 0x33 => Some(KeyboardKey::Num3),
            v if v == 0x34 => Some(KeyboardKey::Num4),
            v if v == 0x35 => Some(KeyboardKey::Num5),
            v if v == 0x36 => Some(KeyboardKey::Num6),
            v if v == 0x37 => Some(KeyboardKey::Num7),
            v if v == 0x38 => Some(KeyboardKey::Num8),
            v if v == 0x39 => Some(KeyboardKey::Num9),
            v if v == 0x41 => Some(KeyboardKey::A),
            v if v == 0x42 => Some(KeyboardKey::B),
            v if v == 0x43 => Some(KeyboardKey::C),
            v if v == 0x44 => Some(KeyboardKey::D),
            v if v == 0x45 => Some(KeyboardKey::E),
            v if v == 0x46 => Some(KeyboardKey::F),
            v if v == 0x47 => Some(KeyboardKey::G),
            v if v == 0x48 => Some(KeyboardKey::H),
            v if v == 0x49 => Some(KeyboardKey::I),
            v if v == 0x4A => Some(KeyboardKey::J),
            v if v == 0x4B => Some(KeyboardKey::K),
            v if v == 0x4C => Some(KeyboardKey::L),
            v if v == 0x4D => Some(KeyboardKey::M),
            v if v == 0x4E => Some(KeyboardKey::N),
            v if v == 0x4F => Some(KeyboardKey::O),
            v if v == 0x50 => Some(KeyboardKey::P),
            v if v == 0x51 => Some(KeyboardKey::Q),
            v if v == 0x52 => Some(KeyboardKey::R),
            v if v == 0x53 => Some(KeyboardKey::S),
            v if v == 0x54 => Some(KeyboardKey::T),
            v if v == 0x55 => Some(KeyboardKey::U),
            v if v == 0x56 => Some(KeyboardKey::V),
            v if v == 0x57 => Some(KeyboardKey::W),
            v if v == 0x58 => Some(KeyboardKey::X),
            v if v == 0x59 => Some(KeyboardKey::Y),
            v if v == 0x5A => Some(KeyboardKey::Z),
